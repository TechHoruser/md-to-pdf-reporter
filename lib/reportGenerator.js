...
function resolveWorkspaceRoot() {
    ...
    for (const candidate of candidates) {
        ...
        // On platforms like Vercel the repo's `public` folder may be the place
        // where static files are served/copied. If `public/base_report/template.html`
        // exists, return the `public` folder as the workspace root so later
        // resolution (workspaceRoot + templateName + 'template.html') works.
        const hasPublicBase = fs.existsSync(path.join(candidate, 'public', 'base_report', 'template.html'));
        if (hasPublicBase) {
            return path.join(candidate, 'public');
        }
    }
