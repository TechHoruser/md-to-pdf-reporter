...
`;
        ...
        if (language === 'mermaid') {
            ...
            const source = toSafeJsString(text || '');

            return `
<div class="mermaid-block" style="margin: 20px 0; page-break-inside: avoid;">
    <div id="${diagramId}" class="mermaid-container"></div>
</div>
<script>
    window.mermaidInitQueue = window.mermaidInitQueue || [];
    window.mermaidInitQueue.push(async function() {
        const container = document.getElementById('${diagramId}');
        if (!container || !window.mermaid) {
            return;
        }
        try {
            const renderResult = await window.mermaid.render('${diagramId}-svg', ${source});
            container.innerHTML = renderResult.svg;
        } catch (err) {
            container.innerHTML = '<pre style="color:#b91c1c; border:1px solid #fecaca; background:#fef2f2; padding:10px; border-radius:8px;">Mermaid render error: ' + String(err && err.message ? err.message : err) + '</pre>';
        }
    });
</script>
`;
...
function resolveTemplatePath(templateName = 'base_report') {
    const roots = [
        process.cwd(),
        __dirname,
    ...
    const requestedName = templateName || 'base_report';
    const fallbacks = requestedName === 'base_report' ? [] : ['base_report'];
    const namesToTry = [requestedName, ...fallbacks];

    const candidates = [];
    for (const root of roots) {
        for (const name of namesToTry) {
            candidates.push(path.resolve(root, name, 'template.html'));
            candidates.push(path.resolve(root, 'public', name, 'template.html'));
        }
    }
    ...
    throw new Error(
        `No template.html found for template "${requestedName}". Checked: ${candidates.join(', ')}`
    );
}
...
function stripLeadingFrontmatter(markdownContent) {
...
function normalizeDashedCodeFences(markdownContent) {
    if (!markdownContent || typeof markdownContent !== 'string') {
        return '';
    }
...
async function launchBrowser() {
    const isServerlessRuntime = Boolean(process.env.VERCEL || process.env.AWS_EXECUTION_ENV || process.env.AWS_REGION);
...
async function renderPdfBuffer({
    ...
    const printableMarkdown = normalizeDashedCodeFences(stripLeadingFrontmatter(markdownContent));
    const renderer = buildRenderer();
    const lexer = new marked.Lexer();
    const tokens = lexer.lex(printableMarkdown);
    const tocHtml = generateTOC(tokens);

    marked.setOptions({ renderer });
    let bodyHtml = marked.parse(printableMarkdown);
...
`;
    ...
    if (bodyHtml.includes('window.mermaidInitQueue')) {
        bodyHtml += `
...
(async function renderMermaidQueue() {
    if (window.mermaidInitQueue && window.mermaidInitQueue.length > 0) {
        for (const renderFn of window.mermaidInitQueue) {
...
`;
    ...
    const hasMermaidDiagrams = bodyHtml.includes('window.mermaidInitQueue');
...
async function generateReportFromReadme({ readmePath, templateName = 'base_report', lang = 'en', browserInstance }) {
    ...
    const coverBase64 = resolveImageAsBase64(path.join(templateDir, 'cover.png'));
    const logoBase64 = resolveImageAsBase64(path.join(templateDir, 'header.png'));
...
async function generateReportFromMarkdown({
    ...
    const fallbackCover = resolveImageAsBase64(path.join(templateDir, 'cover.png'));
    const fallbackHeader = resolveImageAsBase64(path.join(templateDir, 'header.png'));
